...
function App() {
  ...
  async function addJob(event) {
    ...
    if (!online) return;
    const newJob = { ...jobForm, _id: (await (await apiFetch("/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(jobForm) })).json())._id };
  ...
  async function addTask(event) {
    ...
    if (!online) return;
    const newTask = { ...taskForm, completed: false, _id: (await (await apiFetch("/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(taskForm) })).json())._id };
  ...
  async function archiveItem(item, type) {
    ...
    if (online) await apiFetch(`/${collection}/${item._id}`, { method: "DELETE" });
  ...
  async function restoreItem(item) {
    ...
    if (online) await apiFetch(`/${collection}/${item._id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ deleted: false }) });
  ...
  function logout() { localStorage.removeItem("keystone-user"); localStorage.removeItem("keystone-token"); setUser(null); setJobs([]); setTasks([]); setOnline(false); }
  ...
  return <div className="app-shell">
    <header><a className="brand" onClick={() => setPage("overview")}>✦ <span>Keystone</span></a>
      ...
      <div className="header-right"><span className={online ? "connection online" : "connection"}>{online ? "SYNCED" : "OFFLINE"}</span>{user ? <button className="account-button" onClick={logout}>Hi, {user.name} · Log out</button> : <button className="account-button" onClick={() => setAuthMode("login")}>Log in</button>}<button className="add-button" onClick={() => setPage(page === "tasks" ? "tasks" : "jobs")}>＋ Add</button></div>
    </header>
    <main>
